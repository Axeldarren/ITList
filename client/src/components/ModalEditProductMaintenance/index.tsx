...
const ModalEditProductMaintenance = ({ isOpen, onClose, productMaintenance }: Props) => {
  ...
  useEffect(() => {
    if (isOpen && productMaintenance) {
      setName(productMaintenance.name);
      setDescription(productMaintenance.description || "");
      setPriority(productMaintenance.priority || "Medium");
      setStatus(productMaintenance.status);
      setSelectedMaintainers(productMaintenance.maintainers.map(m => m.userId));
    }
  }, [isOpen, productMaintenance]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateProductMaintenance({
        id: productMaintenance.id,
        name: name.trim(),
      ...
      const errorMessage = error && typeof error === 'object' && 'data' in error && 
                          ...
                          : "Failed to update product maintenance";
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} name="Edit Product Maintenance">
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Status */}
        <div>
          <label htmlFor="status" className="mb-2 block text-sm font-medium dark:text-white">
            Status
          </label>
          <select
            id="status"
        ...
        <div>
          ...
          <div className="space-y-2">
            ...
            >
              ...
              {selectedMaintainers.length === 0 ? "Add Maintainer" : "Modify Maintainers"}
            </button>
...
export default ModalEditProductMaintenance;

...
const ViewMembersModal = ({ isOpen, onClose, team }) => {
...
const MultiUserSelector = ({ allUsers, selectedIds, onSelectionChange, poId, pmId }) => {
    const [isOpen, setIsOpen] = useState(false);

    const selectedUsers = useMemo(() => allUsers.filter(u => selectedIds.includes(u.userId)), [allUsers, selectedIds]);
    const availableUsers = useMemo(() => allUsers.filter(u => !selectedIds.includes(u.userId)), [allUsers, selectedIds]);
                ...
                <div className="absolute z-10 w-full mt-1 bg-white dark:bg-dark-secondary border border-gray-300 dark:border-dark-tertiary rounded-md shadow-lg max-h-48 overflow-y-auto">
                    <ul>
                        {availableUsers.length > 0 ? availableUsers.map(user => (
                            <li key={user.userId} onClick={() => { handleSelect(user.userId); setIsOpen(false); }} className="px-4 py-2 hover:bg-gray-100 dark:hover:bg-dark-tertiary cursor-pointer">
                                {user.username}
                            </li>
...
const Teams = () => {
  ...
  const handleUpdateSubmit = (teamData) => {
  ...
  const columns: GridColDef[] = [
    { 
        ...
        headerName: "#", 
        width: 50,
        renderCell: (params) => teams.indexOf(params.row) + 1,
    },
